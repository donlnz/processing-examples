...
int[][] calc(float scale, int iterations, float threshold) {
  ...
  for (int x = 0; x < width; x++) {
    for (int y = 0; y < height; y++) {
      ...
      if (i == iterations) {
        ...
        //result[x][y] = floor(map(sqrt(n), 0, 1, 0, 255));
        result[x][y] = colors[i % colors.length];
      }
    }
  }
  return result;
